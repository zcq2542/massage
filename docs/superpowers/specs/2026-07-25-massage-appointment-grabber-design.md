...
均从站点 JS 的 `save()` 分支确认。`code` 是字符串；`mes` 字段在某些错误码下携带附加数字（如间隔小时数、每周上限）。

| code | 含义 | 处理 |
|---|---|---|
| `> "0"`（正码字符串，如 `"1"`） | **成功** | 置 `won`，停止 |
| `-1` | 预约间隔不足（`mes` = 需间隔的工作时间数） | 通知并结束 |
| `-2` | 该时段已被预约（竞争点） | 立刻回退下一优先时段 |
| `-3` | 手机号格式错 | 配置错误，通知并中止该 profile |
| `-4` | 本周预约次数已达上限（`mes` = 每周最大次数） | 通知并结束；检查 profile 是否超额 / 身份串号 |

> 业务规则（来自成功/`-4` 提示文案）：这是公司员工中医理疗福利，**每位员工每周最多预约 X 次**（X 由 `setting_time[5].remark` 配置）。因此 `-4` 的"按员工计数"身份归属是个关键未知项（见第 11 节）。
...
- [x] 成功 code 条件：`code > "0"`（JS `save()` 已确认）；确切字面值（"1"？）标定时顺手记录
- [x] `-4` 含义：本周预约次数达上限（"每位员工每周最多 X 次"，X = `mes`）
